...
def scale_attention_scores(scores, d_k):
    ...
    return scores / math.sqrt(float(d_k))
...
def softmax_attention_weights(masked_scores):
    ...
    return weights.masked_fill(torch.isnan(weights), 0.0)
...
def split_last_dim_into_heads(tensor, num_heads):
    ...
    return tensor.reshape((batch_size, seq_len, num_heads, d_model//num_heads))
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    batch_size, num_heads, seq_len, d_k = multi_head_tensor.size()
    return multi_head_tensor.transpose(1,2).reshape((batch_size, seq_len, -1))
...
def merge_heads_and_project_output(context, w_o, b_o):
    ...
    context_merged = merge_heads_back_to_model_dim(context)
    return apply_linear_projection(context_merged, w_o, b_o)
...
def assemble_multi_head_attention_forward(query, key, value, w_q, w_k, w_v, w_o, num_heads, mask=None):
    ...
    context, _ = multi_head_scaled_dot_product_attention(q_h, k_h, v_h, mask=mask)
...
def apply_ffn_first_linear_and_relu(x, w1, b1):
    ...
    return torch.nn.functional.relu(torch.matmul(x, w1) + b1)
...
def apply_residual_add_and_norm(residual_input, sublayer_output, gamma, beta, eps=1e-5):
    ...
    pre_layernorm = residual_input + sublayer_output
    return normalize_and_scale_with_gamma_beta(pre_layernorm, gamma, beta, eps=eps)
